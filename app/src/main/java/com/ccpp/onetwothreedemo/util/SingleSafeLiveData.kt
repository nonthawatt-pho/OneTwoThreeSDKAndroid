 ...
 */
class SingleSafeLiveData<T>(val default: T) : MutableLiveData<T>() {
    ...
    inline fun observe(owner: LifecycleOwner, crossinline block: (T) -> Unit) {
        observe(owner, Observer { block(it ?: default) })
    }
    ...
    inline fun observeForever(crossinline block: (T) -> Unit) {
        observeForever(Observer { block(it ?: default) })
    }
}